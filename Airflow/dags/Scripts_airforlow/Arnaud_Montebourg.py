...
def main_montebourg():
    ...
    sc = SparkSession(sc)
    for file in os.listdir(r'/home/sofienkhanfir/Bureau/Projet/nifi-dev/data_candidat_2022/'+candidat):
        list_of_dataframes.append(sc.read.format("csv").option("sep", ",").option("multiline",True).option("header","true").option('encoding','ISO-8859-1').load(r'/home/sofienkhanfir/Bureau/Projet/nifi-dev/data_candidat_2022/'+candidat+'/'+file).select(["id","created_at","user_name","user_location","full_text","user_followers_count","retweet_count"]))
    ...
    union_df.toPandas().to_csv(r'/home/sofienkhanfir/Bureau/Projet/data_candidat/'+candidat+'.csv',index=False)
